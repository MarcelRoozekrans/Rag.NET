...
public class RagBuilderExtensionsTests
{
    ...
    /// <summary>The clusterer's own settings are reachable from the package's public API.</summary>
    /// <remarks>
    /// <b>They were not, and nothing said so.</b> <c>LouvainWithRefinementOptions</c> shipped
    /// public, documented and complete with a resolution parameter — and
    /// <c>CommunityDetectionBehavior</c> called <c>LouvainWithRefinement.Detect(snapshot)</c> with
    /// the argument omitted, so every knob on it was unreachable through <c>UseGraphRag</c> and the
    /// defaults were the only settings that had ever run. That is the exact shape of the three dead
    /// settings audit #108 found.
    /// </remarks>
    [Fact]
    public void UseGraphRag_LouvainWithRefinementOptionsAreConfigurable()
    {
        var services = new ServiceCollection();
        var builder = new RagBuilder(services);
        ...
        {
            o.CommunityDetection.Resolution = 2.5;
            o.CommunityDetection.RandomSeed = 7;
        });

        var opts = services.BuildServiceProvider().GetRequiredService<GraphRagOptions>();
        Assert.Equal(2.5, opts.CommunityDetection.Resolution);
        Assert.Equal(7, opts.CommunityDetection.RandomSeed);
    }

    /// <summary>A clustering setting that would misbehave is rejected where it was configured.</summary>
    /// <remarks>
    ...
    public void UseGraphRag_NonPositiveResolution_ThrowsAtTheConfiguringLine(double resolution)
    {
        var builder = new RagBuilder(new ServiceCollection());

        var ex = Assert.Throws<ArgumentException>(
            () => builder.UseGraphRag(o => o.CommunityDetection.Resolution = resolution));

        Assert.Equal("configure", ex.ParamName);
        Assert.Contains("CommunityDetection", ex.Message, StringComparison.Ordinal);
    }
