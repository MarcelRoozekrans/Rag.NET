...
#pragma warning restore HLQ012
    ...
    /// </summary>
    /// <remarks>
    /// Metadata emitted: <c>drive_id</c> (always, from options) and <c>path</c> — the parent
    /// folder path Graph reports on <c>parentReference</c>. Graph omits <c>parentReference</c> on
    /// some delta payloads, so <c>path</c> is optional and is left out entirely rather than
    /// written empty.
    /// </remarks>
