...
/// <summary>
/// Traag, Waltman and van Eck's Leiden algorithm over modularity: Louvain's local moving and
/// aggregation, with the paper's refinement pass between them constraining what may be aggregated
/// together, so that every community it returns is well connected.
/// </summary>
/// <remarks>
/// <para>
/// <b>What it does.</b> Local moving to a modularity local optimum at
/// <see cref="LeidenOptions.Resolution"/>; then the refinement pass of Traag, Waltman and van Eck
/// (<i>From Louvain to Leiden: guaranteeing well-connected communities</i>, Scientific
...
/// in <see cref="RunLeiden"/>, not of merely having a refinement pass. Only a node alone in its
...
/// with <see cref="LeidenOptions.MaxLevels"/>, and the case where a level's refinement merges
/// nothing at all and so cannot aggregate, both get connected communities by getting a
/// <i>finer</i> partition than the uncapped run would. That is a cost in quality, not in
...
/// randomised search, and a different <see cref="LeidenOptions.RandomSeed"/> is a different local
/// optimum.
/// </para>
/// <para>
/// <b>Where it departs from the paper, and why neither departure touches the guarantee.</b> The
/// local moving phase is a repeated sweep over a shuffled node order rather than the paper's
/// queue-driven <c>MoveNodesFast</c>, which changes how quickly a level converges and not what it
/// converges to. And a node is never offered an empty community to move into, so the set of nodes
/// the refinement will consider can be smaller than the paper's <c>R</c> — which leaves more nodes
/// as refined singletons and costs a level, never connectedness.
/// </para>
/// <para>
/// <b>The name was briefly wrong and is not any more.</b> This type carried all of the construction
/// above except the three refusals the guarantee rests on, and while it did, the name was a
/// claim about a guarantee the code did not have — <c>CommunityConnectivityTests</c> pinned a
/// ten-node weighted tree on which it returned an internally disconnected community at the default
/// resolution, and a sweep found 132 such communities in 3,351,175. It was renamed to
/// <c>LouvainWithRefinement</c> for exactly as long as that was true. The constraints were then
/// implemented (#180), the same sweep returned <b>0 disconnected communities in 3,359,331</b>, and
/// the name came back. Nothing published ever saw the descriptive name, so it left no forwarders
/// behind.
/// </para>
/// </remarks>
public static class Leiden
{
    ...
    /// <see cref="Leiden"/>'s own remarks for where that comes from and for what it
    ...
    /// aggregate graph is the same size as the graph, and <see cref="RunLeiden"/> has to stop a level
