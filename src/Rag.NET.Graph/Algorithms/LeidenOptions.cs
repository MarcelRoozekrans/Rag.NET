...
/// <summary>Options for the <see cref="Leiden"/> community detection algorithm.</summary>
/// <remarks>
/// Sealed, and it can be: it was unsealed only so that a briefly-lived
/// <c>LouvainWithRefinementOptions</c> rename could leave this name behind as an obsolete record
/// deriving from it. Nothing published ever referenced that record, it is gone, and nothing else
/// derives from this type — so the derivation that forced the type open is not a constraint any
/// more. See <see cref="Leiden"/>'s remarks for the rename and what it was about.
/// </remarks>
public sealed record class LeidenOptions
{
    ...
    /// partition, and <c>LeidenTests.Detect_IsDeterministicWithSameSeed</c> asserts
    /// it.
    /// </para>
    /// </remarks>
