...
by graph provenance, not by score, so there is no query that returns them. `InMemoryVectorStore` and
`PgVectorStore` implement it; the remaining remote backends do not yet (#318). Without it, local
