...
`PgVectorStore`, `QdrantVectorStore` and `RedisVectorStore` implement it; the remaining remote
backends do not yet (#318). Note that Redis stores no chunk metadata at all, so its chunks arrive
