...
- **It goes last — for the guards and sanitisers.** It decorates the `IRetrievalGuard`, `IChunkSanitiser` and `IQuerySanitiser` registrations that exist *at the time of the call* — the same ordering rule `ConfigureResilience` and `UseRateLimiting` carry. A guard registered afterwards still runs; it is simply not traced, and its absence from a trace would then read as *"it never fired"*, which is a wrong answer to the question traces exist to answer.
- **The answer is the exception: capturing it is order-independent.** Answer capture is not a registration but a decoration applied when `RagPipeline` composes its engine, so it wraps whatever the container ends up answering with — an `IAnswerEngine` registered before or after this call, or the `ChatAnswerEngine` built from an `IChatClient` registered after `AddRagNet` returned. Before [#195](https://github.com/MarcelRoozekrans/Rag.NET/issues/195) that last shape — the order every walkthrough produces — gave a trace with a query, chunks, stages and a prompt whose `Answer` was always `null` even with `CaptureAnswerText` on, while asking returned an answer perfectly well. Note that resolving `IAnswerEngine` from the container yields the *registered* engine, undecorated; resolve `ComposedAnswerEngine` to get the traced one.
