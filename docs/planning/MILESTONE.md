...
| 6.2.14 | Deletion Reaches the RAPTOR Leaves | #338 | **active** 2026-09-07 — `DeleteAsync` never touched `IRaptorLeafStore`, so under the shipped default scope the next corpus build resurrects a deleted document's text as a searchable summary under `raptor://corpus-tree`, carrying no document id and therefore unremovable. `RemoveDocumentAsync` existed with zero callers. Fixed via a minimal document-scoped abstraction in `Rag.NET.Abstractions`, the pattern the five stores `DeleteAsync` already clears all use; leaves also purge on `Overwrite`, treated as an exception to that behaviour's recorded stranding on the operator's call |
