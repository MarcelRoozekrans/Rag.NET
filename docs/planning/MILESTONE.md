...
| 6.2.5 | Contract Defects | #350, #328, #360 | **pending** — added 2026-08-25. Shipped behaviour that contradicts its own documentation: the BM25 arm of client-side hybrid ignores `MetadataFilter` so a filtered query can return excluded chunks (#350, and wider than first reported — `MinScore` and `EnsembleOptions` take the leaking path too); `KNearestNeighborsCount` hard-coded to `TopK` (#328); and the answer test **passes on an empty query selection**, printing `NaN` and writing a 0-byte sidecar (#360). Grouped by shape, not subsystem: wrong against its own contract, every test green |
| 6.2.6 | Package Boundaries | #339 | **pending** — added 2026-08-25. `Rag.NET.Security` drags `Microsoft.Data.Sqlite` and a native binary for one file, `Audit/SqliteAuditLog.cs`. Pre-tag deliberately: moving a type between packages is breaking and far cheaper before v1.0. Precedent: `Rag.NET.Storage.Sqlite` was extracted the same way at 0.1.0 |
