...
- **The nightly `run-secrets` job now selects hours of work it has 120 minutes for** (found while
  documenting Phase 3.12, from the numbers that phase measured — not observed on a run). `nightly.yml`
  runs `dotnet test` over every `RequiresSecrets` project with **no filter**, and 3.12 added five
  parity cases and three real-chunking cases to that project. Measured: FiQA's parity leg alone is
  **1 h 11 m**, and FiQA's real leg — a case of the same theory, and the one this phase deliberately
  did not run — is **eight to nine hours**. The job's `timeout-minutes` is **120**, and
  `RUNNER_TEMP/beir` is fresh each night, so the embedding cache saves nothing across runs.
  The consequence is the same shape as the inert guards this milestone keeps finding, in the other
  direction: instead of skipping and passing having measured nothing, the job now fails on a timeout
  having measured *some* of it, and a timeout says nothing about parity. Whichever way it lands, the
  nightly stops reporting on the number it exists to report on.
  **Not fixed here, because this was a documentation task and the fix is a decision.** The cheap
  interim is a `--filter` selecting the datasets that fit — `DisplayName~scifact` runs in minutes and
  ArguAna in under an hour. The considered fix is 3.15's cached-embeddings artifact, which is what
  makes FiQA affordable at all. `docs/reference/ci.md` also still describes this job as running "the
  SciFact retrieval-quality parity run", which was true when it was written.
  → **Phase 3.15**, with the caveat that it is live before then: the next nightly is the one that
  finds out.
...
- **FiQA's real-chunking run**, deferred there with a measured cost basis: 429,850 chunks against a parity leg that took 1 h 11 m for 64,247 embeddings, so eight to nine hours plus 429,850 entries sorted per query across 6,648 queries. It is the better test of whether max-pooling helps or hurts than ArguAna's, whose fan-out is mostly the chunker's short-part behaviour. **This phase needs a cached-embeddings artifact regardless**, which is what makes it the natural home.
...
- **A one-line correction to `BeirDatasetDescriptor.FiQA`'s remarks**, which still credit FiQA's 51% of over-long documents with making it "the first dataset where chunk-to-document max-pooling is not a no-op". The protocol makes it a no-op, not the document length, and SciFact exceeds the chunk size more often (99.2%).
- **TREC-COVID** — the first graded-relevance dataset. `IrMetrics` uses `2^rel - 1` and has a graded fixture, but no graded dataset has ever been through it.
- **EnronQA**, for the private-corpus and multi-tenant story.
- **What the nightly runs.** `nightly.yml` selects the whole integration project with no filter and allows it 120 minutes, against cases that now cost hours. The cached-embeddings artifact this phase needs is the considered fix; a `--filter` is the interim. Either way, the job must end up reporting a parity number rather than a timeout.
