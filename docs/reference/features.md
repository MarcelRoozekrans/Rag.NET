...
- **Three bomb caps, and they are counted rather than read.** `ZipArchiveEntry.Length` comes from the central directory, which is written by whoever built the archive, so no cap is enforced by pre-flighting a declared size — every entry is read through a `LimitedReadStream` that counts the bytes it actually produces, and a breach throws `ArchiveLimitExceededException` naming which limit was hit.
