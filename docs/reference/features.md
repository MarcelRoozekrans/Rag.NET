...
**Status:** ✅ Done — shipped, and since 2026-08-12 exercised end to end for the first time. Six defects that first run found are fixed, a seventh (#209, unbounded relationship weights) that only the full corpus could show is fixed with it; what none of it verifies is named below.
...
- **65% of the slice's communities hold one entity** (396 of 607), of which 273 are entities appearing in no relationship at all. The other **123 have edges that never reach the clusterer**: `Leiden.BuildAdjacency` keeps an edge only when both endpoints resolve to an extracted entity, and 853 of the slice's 16,403 relationships (5.20%) fail that — 837 naming something the entity pass never extracted, 16 naming the same entity at both ends. 273 + 123 = 396 exactly. Leiden emitting one community per unattachable node is correct behaviour; it is recorded because the extraction quality underneath it is not something this guard measures. (Over the full 609-article corpus the same drop is 5,492 of 147,021, 3.74%.)
