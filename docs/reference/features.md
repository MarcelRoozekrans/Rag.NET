...
**Status:** ✅ Done — shipped, and since 2026-08-12 exercised end to end for the first time. Six defects that first run found are fixed; two things it did not verify are named below.
**Package:** `Rag.NET.GraphRag`

Full Microsoft GraphRAG pipeline: LLM-driven entity and relationship extraction from chunks using iterative "gleaning", hierarchical community detection via the `LouvainWithRefinement` type (Louvain-family local moving with a refinement pass — **not** the Leiden paper's algorithm; see the type's own remarks; called `Leiden` until 0.1.0, now an `[Obsolete]` forwarder), PageRank-weighted entity scoring, and LLM-generated community summary reports. At query time, combines dense entity retrieval, relation retrieval by text similarity, and community report retrieval — merged and scored by cosine similarity and PageRank.
...
**What is exercised now.** `GraphRagFunctionsTests` (Phase 5.2) runs the whole path — extraction, clustering, PageRank, community reports, local search, global search — over a pinned 60-article slice of MultiHop-RAG, asserting against that dataset's qrels rather than against plausible-looking output. Measured 2026-08-12: 8,999 entities and 16,403 relationships from 60 articles; entities recurring across articles ("Google" in 16 of them); 607 communities, the largest holding **8.8%** of the graph; local search returning a known-relevant document in the top 10 for **all 27** of the slice's judged queries; and global search's map/reduce running over the community reports it found in the *unfiltered* candidate set any caller would get.
...
| The clusterer discarded intra-community weight when aggregating, so merging always paid: ten disjoint 10-node cliques ring-bridged returned **one** community of 100, and on the real slice one community held 89.7% of the graph | `929d45a3` |
...
| `LouvainWithRefinementOptions` was unreachable through the public API while this repository's own guide told readers to tune `Resolution` through it | `c34d270e` |
...
- **No real community report text has ever been retrieved.** The guard's report generation answers through `PromptEchoChatClient`, which returns a bounded head of the prompt, so a "report" there is its community's own entity descriptions rather than the prose a model would write. Whether *real* reports retrieve well is untested.
- **Retrieval-mode routing does not exist.** Issue #104 describes a `Mode` setting selecting local, global or automatic search. Neither the property nor a `GraphRagRetrievalMode` enum is in the package — the one that existed until 0.1.0 was deleted because no behavior read it (#125). Which search runs is a registration decision: add `GraphLocalSearchBehavior`, `GraphGlobalSearchBehavior`, or both.
- **Whether GraphRAG *helps* is unmeasured.** The guard publishes no nDCG and makes no comparison against the dense baseline; that run is deferred in the ROADMAP's Phase 5.2. `BeirReproduction`'s `multihop-rag` / `GraphRag` cell is deliberately empty for the same reason — 60 of 609 documents and 27 of 2,255 queries would not be comparable to anything.
- **65% of the slice's communities hold one entity** (396 of 607), of which 273 are entities appearing in no relationship at all. Emitting one community per unattachable node is correct behaviour; it is recorded because the extraction quality underneath it is not something this guard measures.
- **The clusterer does not guarantee connected communities, and that is measured, not theoretical.** The `LouvainWithRefinement` type — named `Leiden` until 0.1.0, which is the defect — is Louvain-family local moving plus a refinement pass, without the Leiden paper's well-connectedness guarantee (#171). A sweep of ~30,000 detections found disconnected communities only on sparse *weighted* graphs — 48 of 2,220 random weighted trees, none of 2,220 unweighted ones, and none on anything dense — with a ten-node counterexample pinned in `CommunityConnectivityTests` (#180). **Checked on this slice's own graph 2026-08-12: it does not fire** — 0 disconnected of 211 non-singleton communities at the defaults, and 0 across seven resolution/seed settings (1,477 non-singleton communities). **That is an underpowered negative, not a clearance:** 135 of the 211 communities are trees, which is the shape the counterexample lives in, and at the synthetic family's measured per-community rate (53 of 490,358, about 1 in 9,250) the expected number of events in a sample this size is **0.16**. Zero is what you would see either way.
